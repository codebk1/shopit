...
const CartItemSchema = IsarGeneratedSchema(
  schema: IsarSchema(
    name: 'CartItem',
    embedded: true,
    properties: [
  ...
  converter: IsarObjectConverter<void, CartItem>(
    serialize: serializeCartItem,
    deserialize: deserializeCartItem,
  ),
);

@isarProtected
int serializeCartItem(IsarWriter writer, CartItem object) {
  IsarCore.writeString(writer, 1, object.id);
  IsarCore.writeDouble(writer, 2, object.price);
  IsarCore.writeLong(writer, 3, object.quantity);
  return 0;
}

@isarProtected
CartItem deserializeCartItem(IsarReader reader) {
  final String _id;
  _id = IsarCore.readString(reader, 1) ?? '';
  final double _price;
  _price = IsarCore.readDouble(reader, 2);
  final int _quantity;
  _quantity = IsarCore.readLong(reader, 3);
...
extension CartItemQueryFilter
  ...
  QueryBuilder<CartItem, CartItem, QAfterFilterCondition> idLessThanOrEqualTo(
    String value, {
    bool caseSensitive = true,
  }) {
    return QueryBuilder.apply(this, (query) {
